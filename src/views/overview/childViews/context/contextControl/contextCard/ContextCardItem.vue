<template>
  <div class="main" :style='stylerSize()'>
    <div class="slaveC _28 title" :style='stylerSize(100,20,0,18)'>{{ itemInfo.scene_name }}</div>
    <button class="slaveC _28 contextButton" :style='stylerSize(45,15,28,38)' @click="dialogTableVisible = true">{{ $t("GENERAL.CHECK_LIST") }}</button>
    <button class="slaveC _28 contextButton" :style='stylerSize(25,15,38,58)' @click="start">{{ $t("GENERAL.TRIGGER") }}</button>
    ...
    <el-dialog custom-class="dialog-class" :visible.sync="statusVisible" :append-to-body="true">
      <span class="select-item">
        ...
        <!--span >成功發送控制： {{ success_count }} {{$t("GENERAL.COUNT")}} </span-->
