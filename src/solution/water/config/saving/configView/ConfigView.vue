<template>
    <div class="main white_view" :Style="stylerwd()">
        <div class="slaveM" :Style="stylerwd('100','15','0' ,'0')">
            ...
            <div class="slaveC title_item _60" :Style="stylerwd('40','100','40' ,'0')">{{ $t("GENERAL.TARGET_SET_VALUE") }} (m³)</div>
        </div>
    ...
    computed:{
        classwd: function(){
            return function(item){
                let index = add_config.findIndex(e => e.device_ID == item.device_ID)
                if( index != -1){
                    return 'change_item' ;
                }else{
                    return 'text_item' ;
        ...
        async add(){
            ...
            if(add_config.length !== 0){
                for(let i of add_config){
                    console.log(i)
                    let add_res = await setTargetData("water", i["devid"], (Number(i["value"]) * 1000))
            ...
            if(del_config.length !== 0){
                for(let i of del_config){
                    let del_res = await deleteTargetData("water", i["devid"])
                    //console.log(del_res)
