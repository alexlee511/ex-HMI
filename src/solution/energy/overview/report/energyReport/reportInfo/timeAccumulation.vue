<template>
    <div class="main back" :Style='stylerwd()' @click='flag=false'>
		...
		<div class='slaveM chartView overFlowY' :Style='stylerwd(96,65,2,21)'>
            <div class='slaveC _24 text_item borderRB text_center' :Style='stylerwd(10,15,0,0)'>{{$t("GENERAL.DEVICE_NAME")}}</div>
            <div class='slaveC _24 borderRB text_center' :Style='stylermax(0,i)' v-for="(device, i) in reportList">{{device.type}}</div>


            <div class='slaveC _24 text_item borderRB text_center' :Style='stylerwd(10,15,10,0)'>{{$t("GENERAL.LOOP_NAME")}}</div>
            <div class='slaveC _24 text_center' :Style='stylermax(10,i)' v-for="(device, i) in reportList">
                <div class='slaveC _24 borderRB text_center' :Style='stylerwd(100,100/device.loop.length,0,id*(100/device.loop.length))' v-for="(element, id) in device.loop">{{element.name}}</div>
            </div>
        
            <div class="slaveL borderB" :Style='stylerwd(100, 15, 0, 0)'></div>
            <div class="slaveL overFlowX" :Style='stylerwd(60, 15+viewHigh*table_h, 20, 0)'>
                <div class="slaveM borderRB" :Style='stylerwd(20, (100*15)/(15+viewHigh*table_h),0+date*20, 0)' v-for="(title, date) in titleList">
                    <div class="slaveC borderR text_center text_item" :Style='stylerwd(50, 100, 0, 0)' >{{title}}</div>
                    <div class="slaveC text_center text_item" :Style='stylerwd(50, 100, 50, 0)'>{{$t("GENERAL.USE_RATE")}}</div>
                </div>

                
                <div class="slaveM" :Style='stylerwd()' v-for="(device, i) in reportList">
                    <div class='slaveM' :Style='stylerH(id,i)' v-for="(element, id) in device.loop">
                        <div class='slaveC _24 borderRB text_center' :Style='stylerwd(10,100,0+l*20,0)' v-for="(usehour, l) in element.time">{{usehour.hour}}</div>
                        <div class='slaveC _24 text_center borderRB' :Style='stylerwd(10,100,10+m*20,0)' v-for="(userate, m) in element.time">{{userate.useRate}}%</div> 
                    </div>
                    </div>
            
            </div>

                <div class='slaveC _24 text_item borderRB borderL text_center' :Style='stylerwd(10,15,80,0)'>{{$t("GENERAL.PRICE_TOTAL")}}</div>
                <div class='slaveC _24  text_center' :Style='stylermax(80,i)' v-for="(device, i) in reportList">
                    <div class='slaveC _24 borderRB borderL text_center' :Style='stylerwd(100,100/device.loop.length,0,n*(100/device.loop.length))' v-for="(toHour, n) in device.loop">{{toHour.totalHourss}}</div>
                </div>
          

           
                <div class='slaveC _24 text_item borderRB text_center' :Style='stylerwd(10,15,90,0)'>{{$t("GENERAL.USE_RATE")}}</div>
                <div class='slaveC _24  text_center' :Style='stylermax(90,i)' v-for="(device, i) in reportList">
                    <div class='slaveC _24 borderRB text_center' :Style='stylerwd(100,100/device.loop.length,0,o*(100/device.loop.length))' v-for="(toRate, o) in device.loop">{{toRate.totalRatess}}%</div>
                </div>
